...
impl GameLogic for TexasHoldemGame {
    ...
    fn render_game_ui(&self, room: &PublicRoom, _hand: &PrivateHand) -> Element<'static, Msg> {
        let community_cards_display = if !room.community_cards.is_empty() {
            column![
                ...
                Space::with_height(Length::Fixed(4.0)),
                text(self.get_board_phase(&room.community_cards)).size(12).style(|_theme| iced_widget::text::Style {
                    color: Some(iced::Color::from_rgb(0.8, 0.8, 0.5)),
                    ..Default::default()
                }),
            ].spacing(4)
        } else {
            column![
                text("Pre-flop").size(14).style(|_theme| iced_widget::text::Style {
                    color: Some(iced::Color::from_rgb(0.8, 0.8, 0.5)),
                    ..Default::default()
                })
            ]
        ...
        let blinds_info = column![
            text(format!("Small Blind: ${}", room.limit_small)).size(11),
            text(format!("Big Blind: ${}", room.limit_big)).size(11),
        ].spacing(2);

        let game_rules = column![
            text("Texas Hold'em Rules:").size(14).style(|_theme| iced_widget::text::Style {
                color: Some(iced::Color::from_rgb(0.9, 0.9, 0.5)),
                ..Default::default()
            }),
            text("• Use any combination of 2 hole + 5 community cards").size(10),
            text("• 2 hole cards dealt to each player").size(10),
            text("• 5 community cards: Flop (3), Turn (1), River (1)").size(10),
            text("• Pre-flop, Flop, Turn, River betting rounds").size(10),
            text("• Best 5-card poker hand wins").size(10),
        ].spacing(2);
        ...
        .style(|_theme| iced_widget::container::Style {
            background: Some(iced::Background::Color(iced::Color::from_rgba(0.3, 0.1, 0.1, 0.8))),
            border: iced::Border {
    ...
    fn available_actions(&self, room: &PublicRoom, _hand: &PrivateHand, is_your_turn: bool) -> Vec<Element<'static, Msg>> {
        ...
        // Always allow fold (except pre-flop big blind with no raise)
        actions.push(
...
impl TexasHoldemGame {
    fn get_board_phase(&self, community_cards: &[Card]) -> &'static str {
